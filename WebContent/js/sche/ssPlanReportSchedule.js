...
//                    console.log(errorThrown)
    ...
    _drawGanttChart(tableel, dayHours) {
        ...
        copyDatas.forEach((d, i) => {
            const startTime = new Date(`${d.sdate} ${d.stime}:00`).getTime()
            const endTime = new Date(`${d.edate} ${d.etime}:00`).getTime()

            if (new Date(`${d.sdate} ${d.stime}:00`).getTime() >= minT && new Date(`${d.edate} ${d.etime}:00`).getTime() <= maxT) {
                ll.push(d)
...
//        console.log(tr.width - td0.width, (2196.97 - 97.66))
        ...
        ll.forEach(l => {
            if (this.isValidDate(new Date(`${l.sdate} ${l.stime}:00`)) && this.isValidDate(new Date(`${l.edate} ${l.etime}:00`)) && (l.sdate != "" && l.stime != "" && l.edate != "" && l.etime != "")) {
                if ("CONV" === l.ctype) {
                    const lp = getLine(l.sdate, l.stime, l.edate, l.etime) // 시간을 라인으로 변환
                    ...
                    if (finddupl) {
                        ...
                        condPoints.push({x1: lp.x1, x2: lp.x2, y: this.gantt.ylen - 1, ids: [l.rowID]})
                    }
                } else {
                    const lp = getLine(l.sdate, l.stime, l.edate, l.etime) // 시간을 라인으로 변환
                    ...
                    if (finddupl) {
                        ...
                        points.push({x1: lp.x1, x2: lp.x2, y: 0, ids: [l.rowID]})
                    }
                }
            }
        })
        ...
        points.forEach(p => {
            if (p.ids.length != 0) {
                ...
                lineHtml += `<line x1="${p.x1}" y1="${y}" x2="${p.x2}" y2="${y}" stroke="#000099" stroke-width="3px"></line>`
                let tcnum = ""
        ...
        ll.forEach((l, i) => {
            if (this.isValidDate(new Date(`${l.sdate} ${l.stime}:00`)) && this.isValidDate(new Date(`${l.edate} ${l.etime}:00`))) {
                ...
                if ('LOAD' === l.ctype.toUpperCase() && "" !== l.loadrate && "" !== l.readytime) {
                    ...
                    let startDateTime = new Date(`${l.sdate} ${l.stime}`)
