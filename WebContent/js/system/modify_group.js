...
function initI18n() {
	...
	$.i18n.init({
		...
		ns : {
			namespaces : [ 'share', 'modifyGroup' ],
			defaultNs : 'modifyGroup'
		},
		resStore : RES_LANG
	}, function() {
		$('body').i18n();
...
function initGroup() {
	jQuery.ajax({
		...
		success: function(data) {
			...
			if(json.list.length > 0) {
				for(var i = 0; i < json.list.length; i++) {
					var isSel = json.list[i].uid == toUid ? 'selected' : '';
					$('#authGroup').append('<option value="' + json.list[i].uid + '" ' + isSel + '>' + json.list[i].groupName + '</option>');
				}
...
function initUserList() {
	...
	for(var i = 0; i < oldUserList.length; i++) {
		userArr.push(oldUserList[i]);
		
		var text = '' + 
		'<tr id="userTr' + oldUserList[i].uid + '">' + 
		'	<td>' + oldUserList[i].userId + '</td>' + 
		'	<td>' + oldUserList[i].posCode + '</td>' + 
		'	<td>' + oldUserList[i].firstName + ' ' + oldUserList[i].lastName + '</td>' + 
		'	<td class="text-center cursor-pointer"><span onClick="delUser(' + oldUserList[i].uid + ')"><i class="fa fa-close"></i></span></td>' + 
		'</tr>';
...
function updateGroup() {
	...
	if(isEmpty) {
		...
		jQuery.ajax({
			type: 'POST',
			url: contextPath + '/system/group/updateGroup.html',
			traditional: true,
			data: {
				uid: groupUid,
				groupName: title,
			...
			success: function(data) {
				...
				if(json.result == "OK") {
					alertPopBack($.i18n.t('auth.compUpdate'), function() {
						location.href = contextPath + '/system/group.html';
					});
				}else if(json.result == "ECC") {
					alertPopBack($.i18n.t('auth.errCantUpdate'), function() {
						location.href = contextPath + '/system/group.html';
					});
				}else{
