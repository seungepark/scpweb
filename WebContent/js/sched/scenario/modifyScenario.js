...
function initI18n() {
	...
	$.i18n.init({
	    ...
	    ns: {
	      	namespaces: ['share', 'modifyScenario'],
	      	defaultNs: 'modifyScenario'
	    },
	    resStore: RES_LANG
	}, function () {
		$('body').i18n();
...
// 기존 데이터 세팅.
function initData() {
	$('#lngTotal').val(numberFormat(_lngTotal, true, true, true));
	$('#ln2Total').val(numberFormat(_ln2Total, true, true, true));
	$('#margin').val(numberFormat(_margin, true, false, true));
	$('#revenue').val(numberFormat(_revenue, false, false, true));
	$('#cost').val(numberFormat(_cost, false, false, true));
	$('#exRate').val(numberFormat(_exRate, false, true, true));
	...
	if(html != '') {
		...
		for(let i = 0; i < _list.length; i++) {
		  	document.getElementById('scenarioDetailColor' + _list[i].uid).addEventListener('change', function() {
...
function saveScenario() {
	...
	jQuery.ajax({
		type: 'POST',
		url: contextPath + '/sched/scenario/updateScenario.html',
		traditional: true,
		data: {
			uid: _uid,
			description: desc,
