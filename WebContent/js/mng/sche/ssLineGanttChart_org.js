...
class ScheduleManager {
	...
	_ganttBoxSearchSetting() {
		...
		const len = tbody.querySelectorAll("tr")[0].querySelectorAll("td").length
		tbody.insertAdjacentHTML('beforeend', `<tr><td colspan="${len}" class="last-row"></td></tr>`)

		// <button type="button" onclick="searchGantt('search-gantt-form')">Find</button>
		const searchTD = tbody.querySelector("td.last-row")
		...
		searchDiv.setAttribute("class", "search-gantt-form")
		searchDiv.setAttribute("style", "position: sticky; left: 0px;")
		searchInput.setAttribute("style", "width: 100px; height: 25px;")
		searchBtn.setAttribute("button", "button")
		searchBtn.innerText = "Find"

		prevBtn.setAttribute("button", "button")
		prevBtn.innerText = "Move Prev"

		nextBtn.setAttribute("button", "button")
		nextBtn.innerText = "Move Next"
		...
		searchBtn.addEventListener("click", () => {
			...
			if (finds.length != 0) {
				...
				searchSpan.innerText = ` 검색 결과가 없습니다.`
			}
		...
		searchDiv.appendChild(prevBtn)
		searchDiv.appendChild(nextBtn)
		searchDiv.appendChild(searchSpan)
		searchTD.appendChild(searchDiv)
	}
	...
	_drawLineBoxData() {
		...
		const svgroot = `<svg width="100%" height="${this.labels.length * 25 + 40}" xmlns="http://www.w3.org/2000/svg" style="position: absolute; top: 0; left: 0;">
            <style>
            circle {
                cursor:pointer;
            }
            ...
            ${emptyBoxRect}
            ${emptyBoxRect}
            <line x1="0" y1="${4 * 25 + 65}" x2="100%" y2="${4 * 25 + 65}" stroke="red" stroke-width="2px"></line>
		...
		points.forEach((p, i) => {
			...
			circleHtml += `<circle cx="${p.x}" cy="${p.y}" r="5" fill="#000" data-key="${p.id}" pointer-events="all" onclick="moveScheduleBox('${p.id}')" ${p.c ? 'c="' + p.c + '"': '' }></circle>`
		...
		ll.forEach((l, i) => {
			...
			if (this.isValidDate(s) && this.isValidDate(e)) {
				...
				if (td && width !== 0) {
                    ...
                    <div style="position: absolute; left: 0px; top: -21px; font-size: 12px; white-space: nowrap; height: 0px;"> ${l.tcnum} ${l.desc}<span class="time"></span></div>
                    <div row-key="${l.rowID}" class="schedule-box" style="height: 8px; background-color: #${this._getColor(color)}; border: 1px outset #${this._getColor(color)}; border-radius: 5px; ${this.option.isEditable ? 'cursor:pointer;' : ''}"></div>
                </div>`
	...
	_makeLineBoxHtml() {
		return `<div style="height: auto; display:flex; width: 100%;" class="${this.lineBox}">
                ...
                <div style="overflow-x: ${this.option.isGanttChart ? 'hidden' : 'scroll'}; overflow-y: ${this.option.isGanttChart ? 'scroll' : 'hidden'};" class="data">
	...
	_makeGanttBoxHtml() {
		...
		return `<div style="height: ${this.option.style?.height ? this.option.style.height : heightGnt}; display:flex; width: 100%;" class="${this.ganttBox}">
                <div style="height: 100%; min-width: 68px;"></div>
                <div style="overflow-x: scroll; overflow-y: scroll;" class="data">
	...
	_drawTime = (type, dd, j) => {
		...
		for (let i = 0; i < 24; i ++) {
			...
			if (type == "h") {
				html += `<th style="border-left: 0px;" data-key="${dd + "-" + pad }"><span style="position: absolute; left: ${left}px; top: ${top}px;">${pad}</span></th>`
			} else {
