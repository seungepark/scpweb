...
var _detailPopTcUid = 0;		// TC 삭제용 현재 UID.
let _delPostponeUidList = [];	// 자동 순연된 삭제 TC UID 목록.
...
class ScheduleManager {
	...
	_nowScroll() {
		...
		for(let i = 0; i < _dateTimeList.length; i++) {
			if(nowStr == _dateTimeList[i]) {
				...
				let viewArea = document.querySelector('#chartPanel');
				document.getElementById(_dateTimeList[i]).classList.add('v-line-now-head');
				vLineNow.setAttribute('style', 'visibility: visible;');
				vLineNow.setAttribute('style', 'left: ' + (i * 30 + 100) + 'px;');
				
				scrollX.addEventListener("scroll", (event) => {
					let leftPos = i * 30 + 100 - scrollX.scrollLeft;
					
					if(leftPos < viewArea.offsetWidth - 10 && leftPos > 70) {
						vLineNow.setAttribute('style', 'visibility: visible;');
	...
	_drawLineBoxData() {
		...
		points.forEach((p, i) => {
			...
			circleHtml += `<circle cx="${p.x}" cy="${p.y}" r="5" fill="#0671E0" data-key="${p.id}" pointer-events="all" onclick="moveScheduleBox('${p.id}')" ${p.c ? 'c="' + p.c + '"': '' }></circle>`
	...
	_drawGanttBox() {
		...
		table.addEventListener('contextmenu', function (e) {
			...
			let currSeqPosY = e.clientY - 348 + document.getElementById('chartScrollArea').scrollTop;
//			_currSeq = (currSeqPosY / 25) + (currSeqPosY % 25 > 0 ? 1 : 0); // 값 / 25 = 몫, 값 % 25 = 나머지 있으면 +1 : 몫 + 나머지결과
		    ...
		    chartContextMenu.style.left = (e.clientX - 22) + 'px';
		    chartContextMenu.style.top = (e.clientY - 108) + 'px';
				...
				if (td && width !== 0) {
					...
					if(l.flag == 'D') {
						div = `<div data-uid="${l.uid}" class="schedule-root" style="z-index: 20; position: absolute; left: ${parseInt(left) + 30 + ((s.getMinutes() / 60) * 30)}px; top: ${top}px; width: ${width}px;">
				                    ...
				                    <div style="position: absolute; left: ${descX}px; top: -9px; font-size: 11px; font-weight: 400; line-height: 16px; color: var(--neutral-600); white-space: nowrap; height: 0px; text-decoration:line-through;">${l.desc}<span class="time"></span></div>
				                    <div row-key="${l.rowID}" class="schedule-box" style="height: 12px; background-color: #${this._getColor(color)}; border-radius: 2px; ${this.option.isEditable ? 'cursor:pointer;' : ''}"></div>
				                </div>`;
					}
							...
							if (!isRightButton) { // left click event
								...
								const mouseUpHandler = function (e) {
									...
									if (!moveTox && !moveToy) {
										...
										// 250418 추가 //
										if(!isEmpty(tl["sdate"]) && !isEmpty(tl["stime"])) {
											body.querySelector(`[name="sdateTime"]`).value = tl["sdate"] + 'T' + tl["stime"];
										}else {
											body.querySelector(`[name="sdateTime"]`).value = '';
										}
										
										if(!isEmpty(tl["edate"]) && !isEmpty(tl["etime"])) {
											body.querySelector(`[name="edateTime"]`).value = tl["edate"] + 'T' + tl["etime"];
										}else {
											body.querySelector(`[name="edateTime"]`).value = '';
										}
										
										if(!isEmpty(tl["performancesdate"]) && !isEmpty(tl["performancestime"])) {
											body.querySelector(`[name="performancesdateTime"]`).value = tl["performancesdate"] + 'T' + tl["performancestime"];
										}else {
											body.querySelector(`[name="performancesdateTime"]`).value = '';
										}
										
										if(!isEmpty(tl["performanceedate"]) && !isEmpty(tl["performanceetime"])) {
											body.querySelector(`[name="performanceedateTime"]`).value = tl["performanceedate"] + 'T' + tl["performanceetime"];
										}else {
											body.querySelector(`[name="performanceedateTime"]`).value = '';
										}
										//-- 250418 --//

										body.querySelector(`[name="scheTcnum"]`).value = tl["tcnum"];
//										document.getElementById('detailPopTitle').innerText = '[' + $.i18n.t('modal.plan') + '] ' + _gHull + ' ' + _gShipType + (isEmpty(_gKey) ? '' : ' [' + _gKey + ']');
//										document.getElementById('detailPopSubTitle').innerText = tl["tcnum"] + ' ' + body.querySelector(`[name="desc"]`).value;
//										
//										let popCtype = body.querySelector(`[name="ctype"]`).value;
//										let popDtype = body.querySelector(`[name="dtype"]`).value;
//										document.getElementById('detailPopCdType').innerText = (isEmpty(popCtype) ? '-' : popCtype) + '/' + (isEmpty(popDtype) ? '-' : popDtype);
										...
										modal.querySelector(".save").onclick = () => {
											...
											// 250418 추가 //
											let sdateTime = $('#sdateTime').val().split('T');
											let edateTime = $('#edateTime').val().split('T');
											let performancesdateTime = $('#performancesdateTime').val().split('T');
											let performanceedateTime = $('#performanceedateTime').val().split('T');
											tl["sdate"] = !isEmpty(sdateTime) && sdateTime.length == 2 ? sdateTime[0] : '';
											tl["stime"] = !isEmpty(sdateTime) && sdateTime.length == 2 ? sdateTime[1] : '';
											tl["edate"] = !isEmpty(edateTime) && edateTime.length == 2 ? edateTime[0] : '';
											tl["etime"] = !isEmpty(edateTime) && edateTime.length == 2 ? edateTime[1] : '';
											tl["performancesdate"] = !isEmpty(performancesdateTime) && performancesdateTime.length == 2 ? performancesdateTime[0] : '';
											tl["performancestime"] = !isEmpty(performancesdateTime) && performancesdateTime.length == 2 ? performancesdateTime[1] : '';
											tl["performanceedate"] = !isEmpty(performanceedateTime) && performanceedateTime.length == 2 ? performanceedateTime[0] : '';
											tl["performanceetime"] = !isEmpty(performanceedateTime) && performanceedateTime.length == 2 ? performanceedateTime[1] : '';
											//-- 250418 --//
									...
									if (moveTox) {
										...
										if (last != 0) {
											...
											if (td) {
												...
												for(let x = 0; x < copyObj.length; x++) {
													const tempS = new Date(`${copyObj[x].sdate} ${copyObj[x].stime}:00`);
													const tempE = new Date(`${copyObj[x].edate} ${copyObj[x].etime}:00`);
	...
	saveAddTc() {
		...
		/// 250324 - 자동 순연 ///
		if($('#newTcModalIsPostpone').is(':checked')) {
			const newTcS = new Date(newTc.sdate + ' ' + newTc.stime + ':00');
			const newTcE = new Date(newTc.edate + ' ' + newTc.etime + ':00');
			
			if(this.isValidDate(newTcS) && this.isValidDate(newTcE)) {
				const newTcReady = isNanEmpty(newTc.readytime) ? 0 : parseInt(newTc.readytime);
				let newTcDiff = new Date(newTc.sdate + ' ' + newTc.stime + ':00');
				newTcDiff.setMinutes(newTcDiff.getMinutes() - newTcReady);
				
				let isFirstTarget = false;
				let postPoneMin = 0;
				
				for(let i = 0; i < dataTemp.length; i++) {
					const tempS = new Date(dataTemp[i].sdate + ' ' + dataTemp[i].stime + ':00');
					const tempE = new Date(dataTemp[i].edate + ' ' + dataTemp[i].etime + ':00');
					
					if(this.isValidDate(tempS) && this.isValidDate(tempE)) {
						if(!isFirstTarget) {
							const tempReady = isNanEmpty(dataTemp[i].readytime) ? 0 : parseInt(dataTemp[i].readytime);
							let tempDiff = new Date(dataTemp[i].sdate + ' ' + dataTemp[i].stime + ':00');
							tempDiff.setMinutes(tempDiff.getMinutes() - tempReady);
							
							let diffReadyReady = diffMinutes(tempDiff, newTcDiff);
							let diffReadyEnd = diffMinutes(tempDiff, newTcE);
							let diffEndReady = diffMinutes(tempE, newTcDiff);
							let diffEndEnd = diffMinutes(tempE, newTcE);
							
							if((diffEndReady < 0 && diffEndEnd >= 0)			// End : New 범위 내에 있는 경우.
								|| (diffReadyReady <= 0 && diffReadyEnd > 0)	// Start : New 범위 내에 있는 경우.
								|| (diffReadyReady >= 0 && diffEndEnd <= 0)		// Star, End : New 범위 감싸는 경우.
							) {
								let diffCalc = diffMinutes(tempDiff, newTcE);	// New.End - Ready
								
								if(diffCalc > 0) {
									isFirstTarget = true;
									postPoneMin = diffCalc;
								}
							}
						}
						
						if(isFirstTarget) {
							tempS.setMinutes(tempS.getMinutes() + postPoneMin);
							tempE.setMinutes(tempE.getMinutes() + postPoneMin);
							
							const dataKey = tempS.getFullYear() + '-' + this._padTime(tempS.getMonth() + 1) + '-' + this._padTime(tempS.getDate() + '-' + this._padTime(tempS.getHours()));
							
							dataTemp[i].sdate = tempS.getFullYear() + '-' + this._padTime(tempS.getMonth() + 1) + '-' + this._padTime(tempS.getDate());
							dataTemp[i].stime = this._padTime(tempS.getHours()) + ':' + this._padTime(tempS.getMinutes());
							dataTemp[i].edate = tempE.getFullYear() + '-' + this._padTime(tempE.getMonth() + 1) + '-' + this._padTime(tempE.getDate());
							dataTemp[i].etime = this._padTime(tempE.getHours()) + ':' + this._padTime(tempE.getMinutes());
		
							if('R' == dataTemp[i].flag) {
								dataTemp[i].flag = 'U';
							}
						}
					}
	...
	delTc(isPop) {
		let dataTemp = this.getData();
		let dataTemp2 = this.getData();
		let isExist = false;
		let isDelete = false;
		let isPostpone = $('#detailPopIsPostpone').is(':checked');
		let postPoneMin = 0;
		
		for(let i = 0; i < dataTemp.length; i++) {
			let uid = dataTemp[i].uid;
			
			if(uid == _detailPopTcUid) {
				...
				if(flag == 'D') {
					...
					for(let x = 0; x < _delPostponeUidList.length; x++) {
						if(uid == _delPostponeUidList[x]) {
							isPostpone = true;
							_delPostponeUidList.splice(x, 1);
							break;
						}
					}
				}else {
				...
				if(!isPostpone) {
					...
					let tcnum = dataTemp[i].tcnum;
					let sametcnum = dataTemp[i].sametcnum;
					let sdateTime = dataTemp[i].sdate + ' ' + dataTemp[i].stime;
					let edateTime = dataTemp[i].edate + ' ' + dataTemp[i].etime;
					let readytime = isNanEmpty(dataTemp[i].readytime) ? 0 : parseInt(dataTemp[i].readytime);
					let isExistSameTc = false;
					
					for(let x = 0; x < dataTemp2.length; x++) {
						let sDT = dataTemp2[x].sdate + ' ' + dataTemp2[x].stime;
						let eDT = dataTemp2[x].edate + ' ' + dataTemp2[x].etime;
						let readyT = isNanEmpty(dataTemp2[x].readytime) ? 0 : parseInt(dataTemp2[x].readytime);
					
						if(uid != dataTemp2[x].uid && sdateTime == sDT && edateTime == eDT && readytime == readyT) {
							if((tcnum == dataTemp2[x].sametcnum) || (!isEmpty(sametcnum) && sametcnum == dataTemp2[x].tcnum)) {
								isExistSameTc = true;
								break;
							}
						}
					...
					if(isExistSameTc) {
						...
						let tempE = new Date(dataTemp[i].edate + ' ' + dataTemp[i].etime + ':00');
						let tempDiff = new Date(dataTemp[i].sdate + ' ' + dataTemp[i].stime + ':00');
						tempDiff.setMinutes(tempDiff.getMinutes() - (isNanEmpty(dataTemp[i].readytime) ? 0 : parseInt(dataTemp[i].readytime)));
						postPoneMin = diffMinutes(tempDiff, tempE);
						
						if(isDelete) {
							postPoneMin = postPoneMin * -1;
						}
					}
				}
			}else {
