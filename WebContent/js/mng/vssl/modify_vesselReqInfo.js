...
const initI18n = () => {
	...
	$.i18n.init({
		...
		ns: {
			namespaces: ['share', 'mngModVsslReqInfo'],
			defaultNs: 'mngModVsslReqInfo'
		},
		resStore: RES_LANG
	}, function () {
		$('body').i18n();
...
const getVesselReqInfoDetList = () => {
	...
	$.ajax({
		...
		if(textStatus == "success") {
			...
			if (jsonResult.length === 0) emptyData()
	...
	if (row && 0 !== row.uid) {
		const text = `
            <tr data-uid="${row.uid}" data-flag="R">
                ...
                <td class=""><input type="text" name="vsslRIDetName" class=" vsslRIDetName" value="${row.name}" maxlength="100"></td>
                <td class=""><input type="text" name="vsslRIDetRpm" class=" vsslRIDetRpm" value="${row.rpm}" maxlength="4" oninput="this.value = this.value.replace(/[^0-9.-]/g, '').replace(/(\\..*)\\./g, '$1').replace(/(\\--*)\\-/g, '$1');"></td>
                <td class=""><input type="text" name="vsslRIDetLoadrate" class=" vsslRIDetLoadrate" value="${row.loadrate}" maxlength="4" oninput="this.value = this.value.replace(/[^0-9.-]/g, '').replace(/(\\..*)\\./g, '$1').replace(/(\\--*)\\-/g, '$1');" ${row.reqinfotitle == 'DRFTBT' ? "disabled" : ""}></td>
		...
		document.querySelector(`#getVesselReqInfoDetList tr[data-uid='${row.uid}']`).querySelectorAll("input").forEach((n) => {
			n.addEventListener("change", () => {
				n.closest("tr").setAttribute("data-flag", "U")
			})
		})

		document.querySelector(`#getVesselReqInfoDetList tr[data-uid='${row.uid}']`).querySelectorAll("select").forEach((n) => {
			n.addEventListener("change", () => {
				n.closest("tr").setAttribute("data-flag", "U")
			})
		})
...
const saveVesselReqInfoDet = () => {
	...
	if (isEmpty == false) {
		...
		$.ajax({
			type: "POST",
			url: contextPath + "/mng/vssl/insertVesselReqInfoDet.html",
			dataType: 'json',
...
//			$("#dDataPanel").css('max-height', (screen.height * 0.7) + 'px');
			document.getElementById('tbWrap').classList.remove('scroll-area-vessel');
			document.getElementById('tbWrap').classList.add('scroll-area-vessel-hide');
		} else{
			$('#dParentData').show();
			$('#dParentDataHide').text($.i18n.t('showParent'));
//			$("#dDataPanel").css('max-height', (screen.height * 0.5) + 'px');
			document.getElementById('tbWrap').classList.remove('scroll-area-vessel-hide');
			document.getElementById('tbWrap').classList.add('scroll-area-vessel');
		}
	});
